...
use App\Http\Controllers\Facturador\PurchaseClientController;
use App\Http\Controllers\Facturador\PurchaseFlowController;
use App\Http\Controllers\Facturador\Dashboard\GuiaFlowControlController;
...
Route::middleware('auth')->group(function (): void {
    ...
    Route::middleware(['active.company', 'facturador.role', 'facturador.enabled'])
        ...
        ->group(function (): void {
            ...
            Route::get('invoices/{invoice}/payload',  [InvoiceController::class, 'payload'])
                ->name('invoices.payload');
            ...
            // ── Letras de Cambio (Letras por Cobrar de Ventas) ────────────────────────────
            Route::resource('letras', \App\Http\Controllers\Facturador\LetraController::class)
                ->only(['index', 'show'])
                ->names([
                    'index' => 'letras.index',
                    'show'  => 'letras.show',
            ...
            Route::post('letras/{letra}/mark-paid', [\App\Http\Controllers\Facturador\LetraController::class, 'markPaid'])
                ->name('letras.mark-paid');
            Route::delete('letras/{letra}', [\App\Http\Controllers\Facturador\LetraController::class, 'destroy'])
                ->name('letras.destroy');

            // ── Letras de Cambio (Gestión General — Compras) ─────────────────────────────
            // Rutas del LetraCambioController para canje, impresión, etc.
            Route::get('letras/{letraCambio}/imprimir', [\App\Http\Controllers\Facturador\LetraCambioController::class, 'imprimir'])
            ...
            // ── Flujo Compra → Guía → Factura ───────────────────────────────────────────
            Route::get('compras/{purchase}/flujo', [PurchaseFlowController::class, 'show'])
                ->name('purchases.guia-flow');
            
            Route::get('compras/{purchase}/asignar-cliente', [PurchaseClientController::class, 'assignForm'])
                ->name('purchase-client.assign.form');
            Route::post('compras/{purchase}/asignar-cliente', [PurchaseClientController::class, 'assign'])
                ->name('purchase-client.assign');

            // ── Guías de Remisión (Asignación de cliente y generación) ──────────────────────
            Route::get('dashboard/flujo-guias', [GuiaFlowControlController::class, 'index'])
                ->name('dashboard.guia-flow-control');

            Route::get('compras/{purchase}/guia/preview', [\App\Http\Controllers\Facturador\GuiaRemisionController::class, 'preview'])
            ...
            Route::get('clientes/{client}/direcciones', [ClientAddressController::class, 'index'])
                ->name('client-addresses.index');
            Route::post('clientes/{client}/direcciones', [ClientAddressController::class, 'store'])
                ->name('client-addresses.store');
            Route::patch('clientes/{client}/direcciones/{address}', [ClientAddressController::class, 'update'])
                ->name('client-addresses.update');
            Route::delete('clientes/{client}/direcciones/{address}', [ClientAddressController::class, 'destroy'])
                ->name('client-addresses.destroy');
            Route::patch('clientes/{client}/direcciones/{address}/default', [ClientAddressController::class, 'setDefault'])
                ->name('client-addresses.set-default');

            // ── Facturas desde Guía (Nuevo flujo de facturación) ──────────────────────────
            Route::get('guias/{guia}/factura/crear', [InvoiceController::class, 'create_from_guia'])
                ->name('guias.invoices.create');
            Route::post('guias/{guia}/factura', [InvoiceController::class, 'store_from_guia'])
                ->name('guias.invoices.store');
            ...
            // Acciones especiales en cotizaciones
            Route::post('cotizaciones/{quote}/send', [QuoteController::class, 'send'])
                ->name('cotizaciones.send');
            Route::post('cotizaciones/{quote}/versions', [QuoteController::class, 'createVersion'])
                ->name('cotizaciones.versions.create');
            Route::post('cotizaciones/{quote}/to-invoice', [QuoteController::class, 'convertToInvoice'])
                ->name('cotizaciones.to-invoice');
...
// Vista pública de cotizaciones con token UUID
Route::prefix('quotes')->name('quotes.')->group(function (): void {
    Route::get('/{token}', [QuoteClientController::class, 'show'])
        ->name('client.show');
    Route::get('/{token}/pdf', [QuoteClientController::class, 'pdf'])
        ->name('client.pdf');
    Route::post('/{token}/accept', [QuoteClientController::class, 'accept'])
        ->name('client.accept');
    Route::post('/{token}/reject', [QuoteClientController::class, 'reject'])
        ->name('client.reject');
});