  ...
  <form method="POST" action="{{ route('sunat.comprobantes.validar') }}">
    ...
    <div class="sunat-form-grid">
      <div class="sunat-field">
        ...
        <input type="text" name="numRuc" value="{{ old('numRuc') }}" maxlength="11" pattern="[0-9]{11}" required>
      ...
      <div class="sunat-field">
        ...
        <select name="codComp" required>
          @foreach(['01'=>'Factura','03'=>'Boleta de venta','04'=>'Liquidación de compra','07'=>'Nota de crédito','08'=>'Nota de débito','R1'=>'Recibo por honorarios','R7'=>'Nota crédito RH'] as $code => $label)
            <option value="{{ $code }}" @selected(old('codComp', '01') === $code)>{{ $code }} - {{ $label }}</option>
          @endforeach
        </select>
      ...
      <div class="sunat-field">
        <label>Serie</label>
        <input type="text" name="numeroSerie" value="{{ old('numeroSerie') }}" maxlength="4" required>
        @error('numeroSerie')<div class="invalid-feedback">{{ $message }}</div>@enderror
      </div>
      <div class="sunat-field">
        <label>Número</label>
        <input type="number" name="numero" value="{{ old('numero') }}" min="1" max="99999999" required>
        @error('numero')<div class="invalid-feedback">{{ $message }}</div>@enderror
      </div>
      <div class="sunat-field">
        <label>Fecha emisión</label>
        <input type="text" name="fechaEmision" value="{{ old('fechaEmision', now()->format('d/m/Y')) }}" placeholder="dd/mm/aaaa" required>
        @error('fechaEmision')<div class="invalid-feedback">{{ $message }}</div>@enderror
      </div>
      <div class="sunat-field">
        <label>Monto total</label>
        <input type="number" name="monto" value="{{ old('monto') }}" min="0" step="0.01">
