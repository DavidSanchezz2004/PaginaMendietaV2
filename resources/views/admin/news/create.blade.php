...
@section('title', 'Crear Noticia | Portal Mendieta')
        ...
        <div class="module-content-stack">
          ...
          <div class="page-header simple-header">
              <div>
                  ...
                  <p class="page-description">Redactar un nuevo aviso o noticia para los clientes.</p>
              </div>
          ...
          <div class="module-card">
              <form class="module-form form-grid is-editing" id="news-form" method="POST" action="{{ route('news.store') }}" enctype="multipart/form-data">
                  @csrf

                  <div class="form-group" style="grid-column: 1 / -1;">
                      <label for="title" class="form-label" style="color: var(--clr-text-main, #374151);">Título <span style="color:red">*</span></label>
                      <input type="text" class="form-input @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title') }}" required autofocus style="background-color: var(--clr-bg-body, #f9fafb); color: var(--clr-text-main, #111827); border-color: var(--clr-border-light, #e5e7eb);">
                      @error('title')
                  ...
                  <div class="form-group" style="grid-column: 1 / -1;">
                      ...
                      <input type="text" class="form-input @error('excerpt') is-invalid @enderror" id="excerpt" name="excerpt" value="{{ old('excerpt') }}" placeholder="Resumen opcional visible en las tarjetas de noticias." style="background-color: var(--clr-bg-body, #f9fafb); color: var(--clr-text-main, #111827); border-color: var(--clr-border-light, #e5e7eb);">
                      @error('excerpt')
                          <div class="invalid-feedback" style="color: #ef4444; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</div>
                      @enderror
                  </div>
                  ...
                  <div class="form-group" style="grid-column: 1 / -1;">
                      <label for="image" class="form-label" style="color: var(--clr-text-main, #374151);">Imagen de Portada (Opcional)</label>
                      ...
                      <small style="color: var(--clr-text-muted, #6b7280); font-size: 0.75rem;">Recomendado: 1200x630px. Max: 2MB.</small>
                      @error('image')
                          <div class="invalid-feedback" style="color: #ef4444; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</div>
                      @enderror
                  </div>
                  ...
                  <div class="form-group" style="grid-column: 1 / -1;">
                      ...
                      <div class="quill-wrapper @error('content') is-invalid @enderror">
                          <div id="editor-container" style="min-height: 250px; color: var(--clr-text-main, #111827);">{!! old('content') !!}</div>
                      </div>
                  ...
                  <div class="form-group">
                      ...
                      <select class="form-input @error('status') is-invalid @enderror" id="status" name="status" required style="background-color: var(--clr-bg-body, #f9fafb); color: var(--clr-text-main, #111827); border-color: var(--clr-border-light, #e5e7eb);">
                          <option value="draft" {{ old('status') == 'draft' ? 'selected' : '' }}>Borrador (Oculto)</option>
                          <option value="published" {{ old('status', 'published') == 'published' ? 'selected' : '' }}>Publicado (Visible a todos)</option>
                      </select>
                      @error('status')
                          <div class="invalid-feedback" style="color: #ef4444; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</div>
                      @enderror
                  ...
                  <div class="module-actions" style="grid-column: 1 / -1;">
                      ...
                      <button type="submit" class="btn-primary">
                          <i class='bx bx-save'></i> Publicar Noticia
                      </button>
                  </div>
              </form>
          </div>
