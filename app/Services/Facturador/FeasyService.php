...
class FeasyService
{
    ...
    public function sendComprobante(Invoice $invoice): array
    {
        ...
        if (! $result['success'] && $result['http_status'] === 400) {
            ...
            if (
                ! empty($payload['informacion_detraccion']['cuenta_banco_detraccion'])
                && (
                    (string) data_get($result, 'data.codigo_respuesta') === '3034'
                    || str_contains($errorsStr, 'payeefinancialaccount')
                )
            ) {
                $cuenta = (string) $payload['informacion_detraccion']['cuenta_banco_detraccion'];
                $detail = "Se envió cuenta de detracción {$cuenta}, pero Feasy/SUNAT generó el XML con el nodo cac:PayeeFinancialAccount/cbc:ID vacío. Verifique que esa cuenta esté registrada en Feasy > Empresa > MisEmpresas para el RUC emisor {$company->ruc}.";

                $result['errors'][] = $detail;
                $result['message'] = trim(($result['message'] ?? 'Error al enviar documento') . ' - ' . $detail);
            }
        }
    ...
    public function consultar(Company $company, string $tipo, string $serie, string $numero): array
    {
        ...
        $payload = [
            ...
            'numero_documento'             => $this->formatDocumentNumber($numero),
        ];
    ...
    {
        $company = $this->resolveAndValidateCompany($invoice);
        return $invoice->codigo_tipo_documento === '09'
            ? $this->buildGrePayload($invoice, $company)
            : $this->buildPayload($invoice, $company);
    }
    ...
    private function buildPayload(Invoice $invoice, Company $company): array
    {
        ...
        $payload = [
            ...
            'informacion_documento' => [
                ...
                'numero_documento'             => $this->formatDocumentNumber($invoice->numero_documento),
                ...
                'monto_tipo_cambio'            => $this->optionalAmount($invoice->monto_tipo_cambio),
                'monto_descuento_global'       => $this->optionalAmount($invoice->monto_descuento_global ?? null),
                'monto_total_anticipo'         => $this->optionalAmount($invoice->monto_total_anticipo),
                'monto_total_gravado'          => round((float) $invoice->monto_total_gravado, 2),
                'monto_total_inafecto'         => $this->optionalAmount($invoice->monto_total_inafecto),
                'monto_total_exonerado'        => $this->optionalAmount($invoice->monto_total_exonerado),
                'monto_total_exportacion'      => $this->optionalAmount($invoice->monto_total_exportacion),
                'monto_total_descuento'        => $this->optionalAmount($invoice->monto_total_descuento),
                'monto_total_isc'              => $this->optionalAmount($invoice->monto_total_isc),
                'monto_total_igv'              => round((float) $invoice->monto_total_igv, 2),
                'monto_total_impuesto_bolsa'   => $this->optionalAmount($invoice->monto_total_impuesto_bolsa),
                'monto_total_gratuito'         => $this->optionalAmount($invoice->monto_total_gratuito),
                'monto_total_otros_cargos'     => $this->optionalAmount($invoice->monto_total_otros_cargos),
        ...
        // ── Retención (Información de retención de terceros) ──────────────
        $retencion = $this->buildRetentionPayload($invoice);
        if ($retencion !== null) {
            $payload['informacion_retencion'] = $retencion;
            $payload['indicadores'] = array_merge(
                $payload['indicadores'] ?? [],
                ['indicador_retencion' => true]
            );

            Log::channel('stack')->debug('[FeasyService] Retención ENVIADA en payload', [
                'invoice_id'             => $invoice->id,
                'informacion_retencion'  => $payload['informacion_retencion'],
                'indicadores'            => $payload['indicadores'],
            ]);
        }
        ...
        if ($invoice->indicador_detraccion && $invoice->informacion_detraccion) {
            ...
            $payload['informacion_detraccion'] = [
                ...
                'porcentaje_detraccion'         => round((float) ($d['porcentaje_detraccion'] ?? 0), 5),
    ...
    {
        $dep = $this->uppercaseOrNull($company->departamento, 30);
        $pro = $this->uppercaseOrNull($company->provincia, 30);
        $dis = $this->uppercaseOrNull($company->distrito, 30);
        $dir = $this->stringOrNull($company->direccion_fiscal, 200);
    ...
    private function formatDocumentNumber(?string $numero): ?string
    {
    ...
    private function nullableAmount(mixed $value, int $precision = 2): ?float
    {
    ...
    private function optionalAmount(mixed $value, int $precision = 2): ?float
    {
        $amount = $this->nullableAmount($value, $precision);

        return $amount === null || abs($amount) < 0.0000001 ? null : $amount;
    }

    private function stringOrNull(mixed $value, ?int $limit = null): ?string
    {
    ...
    private function uppercaseOrNull(mixed $value, ?int $limit = null): ?string
    {
        $value = $this->stringOrNull($value, $limit);
    ...
    private function buildAdquirientePayload(\App\Models\Client $client): array
    {
        return [
            ...
            'codigo_pais_adquiriente'              => mb_strtoupper($client->codigo_pais ?: 'PE'),
            'ubigeo_adquiriente'                   => $this->stringOrNull($client->ubigeo, 10),
            'departamento_adquiriente'             => $this->uppercaseOrNull($client->departamento, 30),
            'provincia_adquiriente'                => $this->uppercaseOrNull($client->provincia, 30),
            'distrito_adquiriente'                 => $this->uppercaseOrNull($client->distrito, 30),
            'urbanizacion_adquiriente'             => $this->stringOrNull($client->urbanizacion, 100),
            'direccion_adquiriente'                => $this->stringOrNull($client->direccion, 200),
            'correo_adquiriente'                   => $this->stringOrNull($client->correo, 200),
        ];
    }

    /** @return array<string, mixed>|null */
    private function buildRetentionPayload(Invoice $invoice): ?array
    {
        if ($invoice->indicador_detraccion) {
            return null;
        }

        $retInfo = is_array($invoice->retention_info)
            ? $invoice->retention_info
            : json_decode((string) $invoice->retention_info, true);

        $hasStoredRetention = $invoice->retention_enabled || is_array($retInfo) && ! empty($retInfo);
        $shouldAutoRetain = $invoice->codigo_tipo_documento === '01'
            && (bool) ($invoice->client?->is_retainer_agent)
            && (float) $invoice->monto_total > 700;

        if (! $hasStoredRetention && ! $shouldAutoRetain) {
            return null;
        }

        $retInfo = is_array($retInfo) ? $retInfo : [];

        $codigoRetencion = $retInfo['codigo_retencion']
            ?? $retInfo['tipo']
            ?? '62';
        $porcentajeRetencion = (float) (
            $retInfo['porcentaje_retencion']
            ?? $retInfo['porcentaje']
            ?? $invoice->retention_percentage
            ?? 3
        );
        $montoBaseRetencion = (float) (
            $retInfo['monto_base_imponible_retencion']
            ?? $retInfo['base']
            ?? $invoice->retention_base
            ?? $invoice->monto_total
        );
        $montoRetencion = (float) (
            $retInfo['monto_retencion']
            ?? $retInfo['monto']
            ?? $invoice->retention_amount
            ?? 0
        );

        if ($montoBaseRetencion == 0 && $porcentajeRetencion > 0 && $montoRetencion > 0) {
            $montoBaseRetencion = round($montoRetencion * 100 / $porcentajeRetencion, 2);
        }

        if ($montoRetencion == 0 && $montoBaseRetencion > 0 && $porcentajeRetencion > 0) {
            $montoRetencion = round($montoBaseRetencion * $porcentajeRetencion / 100, 2);
        }
        ...
        return [
            'codigo_retencion'                => (string) $codigoRetencion,
            'monto_base_imponible_retencion'  => round($montoBaseRetencion, 2),
            'porcentaje_retencion'            => round($porcentajeRetencion, 5),
    ...
    {
        $cantidad        = round((float) $item->cantidad, 4);
        $precioUnitario  = round((float) $item->monto_precio_unitario, 10);
        $valorTotal      = round((float) $item->monto_valor_total, 2);
        $montoTotal      = round((float) $item->monto_total, 2);

        // Derivar monto_valor_unitario desde monto_valor_total / cantidad para que
        // SUNAT pueda verificar: valorUnitario × cantidad == valorTotal exactamente.
        $valorUnitario = $cantidad > 0
            ? round($valorTotal / $cantidad, 10)
            : round((float) $item->monto_valor_unitario, 10);
        ...
        return [
            ...
            'monto_descuento'        => $this->optionalAmount($item->monto_descuento),
            'monto_valor_total'      => $valorTotal,
            'codigo_isc'             => $item->codigo_isc,
            'monto_isc'              => $this->optionalAmount($item->monto_isc),
            ...
            'monto_impuesto_bolsa'   => $this->optionalAmount($item->monto_impuesto_bolsa),
