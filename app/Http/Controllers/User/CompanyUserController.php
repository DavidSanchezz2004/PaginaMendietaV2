...
class CompanyUserController extends Controller
{
    ...
    public function toggleStatus(Request $request, User $managedUser): RedirectResponse
    {
        $user = $request->user();
        abort_if(! $user, 403);
        abort_if(! $user->can('manageCompanies', Company::class), 403);

        // No se puede bloquear a sí mismo ni a otros administradores
        abort_if($managedUser->id === $user->id, 403);
        $targetRole = $managedUser->role instanceof RoleEnum ? $managedUser->role->value : (string) $managedUser->role;
        abort_if($targetRole === RoleEnum::ADMIN->value, 403);

        $newStatus = ((string) $managedUser->status) === 'active' ? 'inactive' : 'active';
        $managedUser->update(['status' => $newStatus]);

        $label = $newStatus === 'active' ? 'activado' : 'desactivado';

        return redirect()->route('users.index')->with('status', "Usuario {$label} correctamente.");
    }
}
